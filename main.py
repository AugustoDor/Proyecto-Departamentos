...
data = pd.read_csv(r'dataframes/venta_deptos_limpio.csv')
...
@app.get("/recomendar/puntuacion")
def recomendar_puntuacion(
    ...
    recomendaciones = recomendar_por_puntuacion(
        df, rango_precio, peso_ambientes, peso_dormitorios, peso_banios, peso_cocheras
    )
    return recomendaciones.to_dict(orient="records")

# Recomendación por KNN
@app.post("/recomendar/knn")
def recomendar_knn(propiedad: NuevaPropiedad, k: int = 5):
    nueva_propiedad = [
        propiedad.precio,
    ...
    recomendaciones = recomendar_por_knn(df, nueva_propiedad, k)
    return recomendaciones.to_dict(orient="records")